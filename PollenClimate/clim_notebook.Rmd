...
wa_sig <- run_tf(pollen = Pollen, 
                 climate = Climate[,1:12], 
                 func = WA, col = 1, mono = TRUE)

wa_sig[[1]]
```

Weighted Averaging results using monotone deshrinking on subsets of the pollen data indicate that there is significance in reconstructions of summer precipitation ($P_{jja}$) and annual precipitation ($P_{ann}$), and weakly significant evapotranspiration ($E:PE$) & mean warm month temperature ($T_{jja}$).  This means that **winter precipitation signals are not significant**, even if we see significant changes in the reconstructions, we cannot have faith that our training set is able to reconstruct them.  It's not clear how much of this is a function of the training set.  Perhaps if we constrained the training set to a domain that showed stronger influence of winter precipitation, we might be able to capture it better in the model.

##### WAPLS (Four components)

```{r WAPLS_sigtesting_Pine, cache=TRUE, results='asis', echo=FALSE, warning=FALSE}

wapls_sig <- run_tf(pollen = Pollen, climate = Climate[,1:12], func = WAPLS, col = 3) 
...
```
This returns a large number of sites.  We can plot them dynamically using leaflet.  This isn't something you need to do, but it does help showcase the flexibility of R:
...
```{r WA_SigTesting_fpine, cache=TRUE, results='asis', echo=FALSE, warning=FALSE}

run_tf(pollen, fossil = orhid_pol, func = WA, col = 1, mono = TRUE)
...
```{r WAPLS_sigtesting_fPine, cache=TRUE, results='asis', echo=FALSE, warning=FALSE}

run_tf(pollen, fossil = orhid_pol, func = WAPLS, col = 3) 
...
run_tf(pollen, fossil = orhid_pol, func = MAT, col = 2, k = 10) 
...
```{r WA_SigTesting_Fwopine, cache=TRUE, results='asis', echo=FALSE, warning=FALSE}

run_tf(pollen, fossil = orhid_pol, func = WA, drop_pine = TRUE, col = 1, mono = TRUE)

```

WA results again pin $P_{jja}$, and $P_{ann}$, along with $E:PE$, providing support for relatively good reconstruction of these parameters with or without *Pinus* using WA.  Given that these parameters are also significant in the full Modern Pollen Dataset **I believe we have good support for the use of $P_{ann}$ and $P_{jja}$ with WA reconstructions for Orhid.**

#### WAPLS (Four components)

```{r WAPLS_sigtesting_FwoPine, cache=TRUE, results='asis', echo=FALSE, warning=FALSE}

run_tf(pollen, fossil = orhid_pol, func = WAPLS, col = 3, drop_pine = TRUE) 

```

As with the calibration dataset, and the Orhid trial with *Pinus* included, there is significance, but the variance explained is very low.  I don't see a reason to include this method in our results.

#### MAT - ten closest

```{r MAT_sigtesting_FwoPine, cache=TRUE, results='asis', echo=FALSE, warning=FALSE}

run_tf(pollen, fossil = orhid_pol, func = MAT, col = 1, k = 10, drop_pine = TRUE) 

```

MAT again provides support for reconstructions of $P_{jja}$, and $P_{ann}$, but also $P_{djf}$.  The variance explained without *Pinus* for $P_{ann}$ is much lower than with *Pinus* included, likely a function of the utility of *Pinus* as an indicator of total $P_{ann}$ in the European pollen dataset.  There is some support for the use of MAT without *Pinus* to reconstruct $T_{jja}$, but I'm not sure how supported 10% of variance is, given than nearly 39% of variance can be explained when *Pinus* is included in the dataset.
...
```{r Pine_reconst, cache=TRUE, results='asis', echo=FALSE, warning=FALSE}
...
```{r plot_Pine_reconst, fig.width=8, fig.height=9, echo=FALSE}
...
```{r no_Pine_reconst, cache=TRUE, results='asis', echo=FALSE, warning=FALSE}

# I'm getting a warning that some pollen_pct_np is showing zero dists, but I don't see them. . . 
if ("mat_reconst_np.RDS" %in% list.files("data/output")) {
  mat_reconst_np <- readRDS(file = 'data/output/mat_reconst_np.RDS')
} else {
  mat_reconst_np <- lapply(1:8, function(x) { predict(rioja::MAT(y = pollen_pct_np, 
                                                              x = clim[,x], 
                                                              k = 10, lean = FALSE),
                                                   newdata = orhid_pol, sse = TRUE) })
  saveRDS(mat_reconst_np, file = 'data/output/mat_reconst_np.RDS')
}
  
if ("wa_reconst_np.RDS" %in% list.files("data/output")) {
  wa_reconst_np <- readRDS(file = 'data/output/wa_reconst_np.RDS')
} else {
  wa_reconst_np <-  lapply(1:8, function(x) { predict(rioja::WA(y = pollen_pct_np, 
                                                              x = clim[,x], 
                                                              mono = TRUE, lean = FALSE),
                                                   newdata = orhid_pol, sse = TRUE) })
  saveRDS(wa_reconst_np, file = 'data/output/wa_reconst_np.RDS')
}

if ("wapls_reconst.RDS" %in% list.files("data/output")) {
  wapls_reconst_np <- readRDS(file = 'data/output/wapls_reconst_np.RDS')
} else {
  wapls_reconst_np <- lapply(1:8, function(x) { predict(rioja::WAPLS(y = pollen_pct_np, 
                                                                  x = clim[,x], 
                                                                  npls = 4, lean = FALSE),
                                                     newdata = orhid_pol, sse = TRUE) })
  saveRDS(wapls_reconst_np, file = 'data/output/wapls_reconst_np.RDS')
}

```

```{r plot_reconst_wop, fig.width=8, fig.height=9, echo=FALSE}

# Now make a huge data.frame:
get_clim_wop <- lapply(1:8, function(x) { 
      data.frame(age = 1:nrow(wa_reconst_np[[x]]$fit),
                 reconst = c(wa_reconst_np[[x]]$fit[,3],
                             wapls_reconst_np[[x]]$fit[,4],
                             mat_reconst_np[[x]]$fit[,1]),
                 err     = c(wa_reconst_np[[x]]$SEP[,3],
                             wapls_reconst_np[[x]]$SEP[,4],
                             mat_reconst_np[[x]]$SEP[,1]),
                 model   = rep(c("WA", "WAPLS", "MAT"), 
                               each = length(orhid_ages)),
                 climate = colnames(clim)[x])}) %>% 
      do.call(rbind.data.frame,.)

ggplot(get_clim_wop, aes(x = age, y = reconst)) + geom_line(aes(col = model)) +
  geom_ribbon(aes(ymin = reconst - err, ymax = reconst + err, group = model), alpha = 0.2) +
  facet_wrap(~climate, scales = 'free', ncol = 2) +
  xlab("Age - kyr BP") +
  ylab("Reconstructed Parameter") +
  ggtitle("Without Pinus")

write.csv(reshape2::dcast(get_clim, age ~ model + climate, value.var = "reconst"), 
          "data/output/w_pine_reconst.csv", row.names = orhid_ages)
write.csv(reshape2::dcast(get_clim, age ~ model + climate, value.var = "err"), 
          "data/output/w_pine_err.csv", row.names = orhid_ages)

write.csv(reshape2::dcast(get_clim_wop, age ~ model + climate, value.var = "reconst"), 
          "data/output/wo_pine_reconst.csv", row.names = orhid_ages)
write.csv(reshape2::dcast(get_clim_wop, age ~ model + climate, value.var = "err"),
          "data/output/wo_pine_err.csv", row.names = orhid_ages)
